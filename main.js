...
function preload()
{
  ball_paddle_wav = loadSound("ball_touch_paddel.wav");
...
function gotPoses(results)
{
  if(results.length > 0)
  {
    ...
    console.log("Nose X = " + rightWristX + " Right Wrist Y = " + rightWristY + "Right Wrist Score = " + wristscore);
  }
}
...
function move(){
  ...
  if (ball.y >= paddle1Y&& ball.y <= paddle1Y + paddle1Height) {
    ball.dx = -ball.dx+0.5; s
    ball_paddle_wav.play();
  }
  else{
    pcscore++;
    reset();
